    ...
    return {
        ...
        async load(id, options) {
            ...
            const wasmUrlDeclaration = options?.ssr || runningInVitest
                ? `const __vite__wasmUrl = ${JSON.stringify(await (0, util_1.createBase64UriForWasm)(moduleId))}`
                : `import __vite__wasmUrl from ${JSON.stringify(wasmUrlUrl)}`;
            return `
URL = globalThis.URL
${wasmUrlDeclaration}
import __vite__initWasm from "${wasmHelper.id}"
${await (0, wasm_parser_1.generateGlueCode)(moduleId, { initWasm: "__vite__initWasm", wasmUrl: "__vite__wasmUrl" })}
`;
        }
    };
}
